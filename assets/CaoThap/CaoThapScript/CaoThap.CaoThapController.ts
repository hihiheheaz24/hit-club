...
export default class CaoThapController extends MiniGame {
    ...
    showListSprAt(index){
        for (let i = 0; i < this.listSprAt.length; i++) {
            const sprAt = this.listSprAt[i];
            sprAt.node.active = false;
        }
        
        if(index <= this.listSprAt.length){
            for (let i = 0; i < index; i++) {
                const sprAt = this.listSprAt[i];
                sprAt.node.active = true;
            }
        }   
    }
