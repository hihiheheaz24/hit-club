...
export default class PopupTransaction extends Dialog {
    ...
    convertDate(dateString = "2010-08-09 01:02:03"){
    ...
    updateTableData(listData = []){
        ...
        this.table.children.forEach((item, index) => {
            if (index < listData.length) {
                ...
                const stringDate = this.convertDate(data['CreatedAt'])/1000;
                item.getChildByName('lbID').getComponent(cc.Label).string = "" + Math.ceil(stringDate);
                ...
                item.getChildByName("lbType").getComponent(cc.Label).string = this.type == TYPE.RUT ? "Rút Internet\nBanking": "Nạp CodePay";
                item.getChildByName("lbNote").getComponent(cc.Label).string = data['Description'] ? data['Description'] : "_";
