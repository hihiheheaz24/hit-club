...
export default class TaiXiuMiniController extends cc.Component {
            ...
            switch (inpacket.getCmdId()) {
                ...
                case cmd.Code.GAME_INFO: {
                    ...
                    if (res.bettingState) {
                        ...
                        this.lblRemainTime2.string = !res.remainTime ? "0" : "" + res.remainTime;
                        this.showResult();
                    }
                    if (!res.bettingState) {
                    ...
                    if(res['fundJpXiu']){
                        Tween.numberTo(this.lbJackPotXiu, Math.round((res['fundJpTai'] + res['fundJpXiu']) / 2), 1.0);
                    }
                    //cc.warn("INFO", res)
                    break;
                }
                case cmd.Code.UPDATE_TIME: {
                    ...
                    //cc.warn("UPDATE_TIME", res)
                    if (res.bettingState) {
                        //cc.warn("UPDATE_TIME BETTING", res['fundJpXiu'])
                        ...
                        //cc.warn("UPDATE_TIME NOT BETTING", res['fundJpXiu'])
                        ...
                        if(!this.isNan && res.remainTime < 10){
                            //cc.warn("KO NAN")
                            if(res['fundJpXiu']){
                                Tween.numberTo(this.lbJackPotXiu, Math.round((res['fundJpTai'] + res['fundJpXiu']) / 2), 1.0);
                            }
                        ...
                        if(this.isNan && this.isOpenBowl){
                            //cc.warn("NAM MO DO KEO")
                            if(res['fundJpXiu']){
                                Tween.numberTo(this.lbJackPotXiu, Math.round((res['fundJpTai'] + res['fundJpXiu']) / 2), 1.0);
                            }
                        }
                ...
                case cmd.Code.DICES_RESULT: {
                    ...
                    this.arrTimeoutDice.push(setTimeout(() => {
                        ...
                        //cc.warn("DICES_RESULT", res['fundJpXiu'])
                        this.lastScore = res.dice1 + res.dice2 + res.dice3;
                        this.lblRemainTime.node.active = false;
                ...
                case cmd.Code.NEW_GAME: {
                    ...
                    // if(res['fundJpXiu']){
                    //     Tween.numberTo(this.lbJackPotXiu, res['fundJpXiu'], 1.0);
                    // }
                    //cc.warn("NEW_GAME", res)
                ...
                case cmd.Code.JACKPOT: {
                    ...
                    //cc.warn("JACKPOT", this.jackpotData)
    ...
    actBetTai() {
        ...
        cc.tween(this.layoutBet).to(0.01, { y: -285 }, { easing: cc.easing.sineOut }).start();
    ...
    actPopupTopHu() {
        this.playSoundClick();
        App.instance.showBgMiniGame("TaiXiu");
        if (this.popupTopHu == null) {
            this.popupTopHu = cc.instantiate(this.popupsPr[5]).getComponent("TaiXiuMini.PopupTopHu");
            this.popupTopHu.node.parent = this.popupContainer;
            this.popupTopHu.show();
            this.popups.push(this.popupTopHu.node);
            App.instance.showLoading(false);
        } else {
            this.popupTopHu.show();
        }
    ...
    sendChat(message: string) {
        let _this = this;
        if (!_this.isCanChat) {
            this.showToast("Mỗi lần chat phải cách nhau 7s");
            return;
        }
