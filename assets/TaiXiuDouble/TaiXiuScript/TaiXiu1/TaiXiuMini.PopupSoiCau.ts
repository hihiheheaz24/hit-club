    ...
    export class PopupSoiCau extends Dialog {
        ...
        private drawPage1() {
            ...
            for (var i = data.length - 1; i >= 0; i--) {
                ...
                iconXX123.getComponentInChildren(cc.Label).node.active = true;
            ...
            if (count > 1) {
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    if (_isTai !== isTai) {
                        ...
                        // }
                        // data.push(curData);
                        let _maxItem = maxItem;
                        let _curData = [];
                        let _data = [];
                        curData = curData.reverse();
                        for (let b = 0; b < curData.length; b++) {
                            _curData.push(curData[b]);
                            _maxItem--;
                            if (_maxItem <= 0 || b == curData.length - 1) {
                                _data.push(_curData.reverse());
                                _maxItem = maxItem;
                                _curData = [];
                            }
                        }
                        _data.reverse().forEach(e=>{
                            data.push(e);
                        });
                        if (isTai) {
