...
@ccclass
export default class TaiXiuDoubleController extends MiniGame {c
    ...
    public onLoad() {
        ...
        if(this.chatNode){
            this.chatNode.on(cc.Node.EventType.TOUCH_START, (event: cc.Event.EventTouch) => {
                // App.instance.showBgMiniGame(this.nameGame);
            }, this);
    
            this.chatNode.on(cc.Node.EventType.TOUCH_MOVE, (event: cc.Event.EventTouch) => {               
                if(this.scrollViewChat.isScrolling()) return;
                var pos = this.chatNode.position;
                pos.x += event.getDeltaX();
                pos.y += event.getDeltaY();
                this.chatNode.position = pos;
            }, this);
        }
