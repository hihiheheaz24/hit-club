...
export default class PopupDetailHistory extends Dialog {
    ...
    private loadData() {
        ...
        this.lblSession.string = "Phiên: #" + this.session;
        this.lblTime.string = "";
        Http.get(Configs.App.API, { "c": 102, "rid": this.session, "mt": Configs.App.MONEY_TYPE, "type":"md5" }, (err, res) => {
            if (err != null) return;
            this.historiesTai = [];
            this.historiesXiu = [];
            if (res.success && res["resultTX"] !== null) {
                ...
                this.lblTotalBetTai.string = Utils.formatMoney(this.totalBetTai);
                this.lblTotalBetXiu.string = Utils.formatMoney(this.totalBetXiu);
    ...
    private loadDataPage() {
        for (var i = 0; i < this.items.length; i++) {
            ...
            if (idx < this.historiesTai.length) {
                ...
                item.getChildByName("Refund").getComponent(cc.Label).string = Utils.formatMoney(itemData["refund"]);
                item.getChildByName("Bet").getComponent(cc.Label).string = Utils.formatMoney(itemData["betValue"]);
            } else {
            ...
            if (idx < this.historiesXiu.length) {
                ...
                item.getChildByName("Refund2").getComponent(cc.Label).string = Utils.formatMoney(itemData["refund"]);
                item.getChildByName("Bet2").getComponent(cc.Label).string = Utils.formatMoney(itemData["betValue"]);
            } else {
